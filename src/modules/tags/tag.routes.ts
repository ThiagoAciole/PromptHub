...
export const tagRoutes: FastifyPluginAsync = async (app) => {
  app.get("/", async () => app.db.select({ id: tags.id, name: tags.name, slug: tags.slug, createdAt: tags.createdAt, updatedAt: tags.updatedAt, promptCount: sql<number>`(select count(*)::int from ${promptTags} where ${promptTags.tagId} = ${tags.id})` }).from(tags).orderBy(asc(tags.name)));
  app.post<{ Body: { name: string; slug: string } }>("/", { schema: { body: bodySchema } }, async (request, reply) => {
    try {
      const [tag] = await app.db.insert(tags).values({ ...request.body, slug: normalizeSlug(request.body.slug) }).returning();
  ...
  app.patch<{ Params: { id: string }; Body: Partial<{ name: string; slug: string }> }>(
    ...
    async (request) => {
      const [tag] = await app.db.update(tags).set({ ...request.body, ...(request.body.slug === undefined ? {} : { slug: normalizeSlug(request.body.slug) }), updatedAt: new Date() }).where(eq(tags.id, request.params.id)).returning();
      if (!tag) throw new AppError("NOT_FOUND", 404, "Tag não encontrada");
      return tag;
    }
  );
