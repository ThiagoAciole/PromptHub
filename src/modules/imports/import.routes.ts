...
export const importRoutes: FastifyPluginAsync = async (app) => {
  app.post("/csv", async (request, reply) => {
    ...
    if (!acceptedMimeTypes.has(file.mimetype)) return reply.code(400).send({ error: { code: "IMPORT_INVALID_MIME", message: "O arquivo deve ter MIME type CSV", statusCode: 400, details: null } });
